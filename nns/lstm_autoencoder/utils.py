...
def evaluate(evaluation_data, condition_seqs, args, scaler, params_etcs, models):
    ...
    with torch.no_grad():
        ...
        for day in range(params_etcs["prediction_window"]):
            samples = np.array([])
            samples_ae = np.array([])
            # Saving the hidden cells of the autoencoder so they can be reset 
            # every iteration.
            encoder_hidden_cell = deepcopy(model_ae.hidden_cell_ae)
            decoder_hidden_cell = deepcopy(model_ae.hidden_cell_de)
            # Likewise with the predictive network
            predictive_hidden_cell = deepcopy(model_pn.hidden_cell)
            ...
            for i in range(100):
                ...
                samples = np.append(samples, scaler.inverse_transform(model_pn(seq).numpy().reshape(-1, dim))).reshape(i+1, -1, dim)
