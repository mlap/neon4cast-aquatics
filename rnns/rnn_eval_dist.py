  ...
  for i in range(10):
    ...
    for i in range(fut_pred):
      ...
      with torch.no_grad():
        ...
        means = np.append(means, scaler.inverse_transform(samples).mean(axis=0)).reshape(-1, 4)
        stds = np.append(stds, scaler.inverse_transform(samples).std(axis=0)).reshape(-1, 4)
