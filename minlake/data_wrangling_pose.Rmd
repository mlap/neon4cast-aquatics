...
waterT_cleaned <- waterT %>%
  ...
  filter(siteID == "POSE") %>% 
  collect() %>% 
  summarise(groundwaterTempMean = mean(groundwaterTempMean, na.rm = TRUE), .groups="drop") %>% 
  select(month, day, year, groundwaterTempMean)
```

```{r}
turbidity_cleaned <- waq %>%
  select(siteID, startDateTime, turbidity) %>%
  ...
  mutate(date = as_date(startDateTime),
         ...
         day = day(startDateTime)) %>%
  group_by(siteID, month, day, year, .groups="drop") %>% 
  summarize(turbidity = mean(turbidity, na.rm = TRUE), .groups="drop") %>% 
  filter(siteID == "POSE") %>% 
  select(month, day, year, turbidity) 
```


```{r}
oxy_cleaned <- oxy_cleaned %>% collect()
chla_cleaned <- chla_cleaned %>%  collect()
turbidity_cleaned <-  turbidity_cleaned %>% collect()
csv <- inner_join(oxy_cleaned, chla_cleaned, by=c("month", "day", "year"))
csv <- waterT_cleaned %>% inner_join(csv, by=c("month", "day", "year"))
csv <- turbidity_cleaned %>% inner_join(csv, by=c("month", "day", "year"))
csv <- csv %>% collect()
write.csv(csv, "minlake_test_POSE.csv")
```
