...
oxy_cleaned <- waq %>%
  ...
  summarize(dissolvedOxygen = mean(dissolvedOxygen, na.rm = TRUE)) %>% 
  filter(siteID == "BARC") %>% 
...
waterT_cleaned <- waterT %>%
  ...
  mutate(date = as_date(startDateTime),
         ...
         day = day(startDateTime)) %>%
  group_by(siteID, month, day, year, .groups="drop") %>% 
  summarize(groundwaterTempMean = mean(groundwaterTempMean, na.rm = TRUE)) %>% 
  filter(siteID == "BARC") %>% 
...
PARbelow_cleaned <- PARbelow %>% 
  ...
  mutate(date = as_date(startDateTime),
         ...
         day = day(startDateTime)) %>%
  group_by(siteID, month, day, year, .groups="drop") %>% 
  summarize(uPARMean = mean(uPARMean, na.rm = TRUE)) %>% 
  filter(siteID == "BARC") %>% 
...
write.csv(csv, "minlake_test.csv")
```